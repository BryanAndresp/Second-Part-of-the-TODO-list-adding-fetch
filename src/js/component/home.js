...
export function Home() {
	...
	useEffect(() => {
		...
		const getTask = () => {
			fetch(
				...
				.then(resp => resp.json())
				.then(data => {
					data.msg ? createUser() : setTasks(data);
				})
				.catch(error => console.log(error));
		};
		getTask();
	...
	const addNewTask = currentTask => {
		fetch("https://assets.breatheco.de/apis/fake/todos/user/alesanchezrz", {
			method: "POST",
					...
					<ul className="list-group">
						{task === null
							...
							: task.map((item, index) => {
									return (
										<li
											className="row list-group-item d-inline-flex align-items-center"
											id={index}
											key={index}>
											...
											<div className="col-1 btn">
												<span
													type="button"
													className="btn btn-danger fas fa-trash-alt text-end"
													onClick={() =>
														deleteTask(index)
													}></span>
											</div>
